...
Dedup layers (in order):
  Layer 1: Base Skip List — domain or company name
  Layer 2: Base historical dedup — fuzzy match against Customers + shipped Leads
           via kb.already_seen()
  Layer 3: Sales Nav CRM check (sales_nav_check.check) — BD-team-wide CRM
...
def _load_skip_list(cfg) -> set[str]:
    ...
    if base_sync is None:
        print(
            "[dedup] WARN: lark_base_sync.py unavailable; Base Skip List disabled for this run",
            file=sys.stderr,
        )
        return set()
    try:
        if not base_sync.is_configured(cfg):
            print(
                "[dedup] WARN: Lark Base is not configured; Base Skip List disabled for this run",
                file=sys.stderr,
            )
            return set()
        return set(base_sync.read_skip_list(cfg))
    except Exception as e:
        print(
            f"[dedup] WARN: failed to read Lark Base Skip List: {e}; treating skip list as empty",
            file=sys.stderr,
        )
        return set()
...
def _kb_already_seen(name: str, cfg) -> bool:
    """Layer 2: fuzzy historical dedup via Base Customers + shipped Leads."""
    kb = _kb()
    if kb is None:
        return False
    try:
...
def _append_skip_list(cfg, entries: list[tuple[str, str]]) -> None:
    ...
    if base_sync is None:
        print(
            "[dedup] WARN: lark_base_sync.py unavailable; could not append Base Skip List entries",
            file=sys.stderr,
        )
        return
    try:
        if not base_sync.is_configured(cfg):
            print(
                "[dedup] WARN: Lark Base is not configured; could not append Base Skip List entries",
                file=sys.stderr,
            )
            return
        base_sync.append_skip_entries(cfg, entries, date_value=_today_myt())
    except Exception as e:
        print(f"[dedup] WARN: failed to append Base Skip List entries: {e}", file=sys.stderr)
...
def run_dedup(topic: str, cfg, dry_run: bool = False) -> int:
    ...
    # 3-layer dedup stack:
    #   Layer 1 (Base Skip List)                — cheapest, in-memory set
    #   Layer 2 (Base Customers + shipped Leads) — fuzzy historical dedup
    #   Layer 3 (Sales Nav)                     — personal Sales Nav session check; if unavailable,
    #                         continue and flag rows for manual CRM review
    skip_set = _load_skip_list(cfg)

    survivors: list[dict] = []
    new_skip_entries: list[tuple[str, str]] = []
    counts = {"layer1": 0, "layer2": 0, "layer3_in_crm": 0, "layer3_not_in_crm": 0,
    ...
    for c in candidates:
        ...
        if nname in skip_set or (domain and _normalize(domain) in skip_set):
            counts["layer1"] += 1
            _log(f"  SKIP layer1 (Base Skip List): {name}", cfg)
            continue
        if _kb_already_seen(name, cfg):
            counts["layer2"] += 1
            _log(f"  SKIP layer2 (Base historical dedup): {name}", cfg)
            continue

        if dry_run:
            _log(f"  [dry-run] would layer3 query (Sales Nav): {name}", cfg)
            survivors.append(c)
            continue

        result = sales_nav_check(name, cfg)
        layer3_unverified = False
        if result.get("error") == "sso-expired":
            _log(f"  layer3 session-expired (Sales Nav): {name} — including, flagged for manual review", cfg)
            counts["layer3_not_in_crm"] += 1
            c["sales_nav_not_found"] = True
            layer3_unverified = True
            ...
            _log(f"  layer3 not-found (Sales Nav): {name} — including, flagged for manual review", cfg)
            counts["layer3_not_in_crm"] += 1
            c["sales_nav_not_found"] = True
            layer3_unverified = True
        elif result.get("error"):
            _log(f"  WARN layer3 error for {name}: {result.get('error')}", cfg)
            counts["errors"] += 1
            continue
        elif result.get("in_crm"):
            counts["layer3_in_crm"] += 1
            new_skip_entries.append((name, f"sales-nav: in CRM"))
            _log(f"  SKIP layer3 (Sales Nav: in CRM): {name}", cfg)
            continue

        if not layer3_unverified:
            counts["layer3_not_in_crm"] += 1
